<template>
        ...
        <div class="flex flex-1 items-center justify-between">
          <NuxtLink to="/">
            <div class="logo flex items-center">
              <img width="48" height="48" class="mr-6 hidden overflow-hidden rounded-md md:block" src="/logo.png"
                alt="earth-worm-logo" />
          ...
          <nav v-if="route.path === '/' && !isAuthenticated()" aria-label="Global" class="hidden md:block">
            <ul class="flex items-center text-base">
              <li class="px-4" v-for="(optItem, optIndex) in HEADER_OPTIONS" :key="optIndex">
                <a class="text-nowrap hover:text-purple-600 dark:text-white dark:hover:text-purple-400"
                  :href="optItem.href" :target="optItem.target ?? optItem.target">
                  {{ optItem.name }}
                </a>
              </li>
            </ul>
  ...
  <UserMenu v-model:open="isOpenUserMenu" @logout="handleLogout" />
  <MainMessageBox v-model:show-modal="isShowModal" content="是否确认退出登录？" confirm-btn-text="确认" @confirm="signOut" />
</template>
